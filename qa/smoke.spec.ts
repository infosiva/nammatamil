...
test('primary CTA visible', async ({ page }) => { await page.goto('/'); await expect(page.locator('a,button').filter({hasText:/read|explore|latest|news|செய்தி/i}).first()).toBeVisible({timeout:10_000}); });

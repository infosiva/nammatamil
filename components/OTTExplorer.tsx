...
import { Star, Play, Tv2, Calendar, Search, X, ChevronDown, ChevronUp } from 'lucide-react'
...
const GENRES = ['All', 'Action', 'Drama', 'Thriller', 'Romance', 'Historical', 'True Story', 'Comedy']
...
function PlatformBadge({ platform }: { platform: string }) {
  ...
  if (!c) return <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/8 text-slate-400">{platform}</span>
  return (
    <span
...
export default function OTTExplorer() {
  ...
  const results = useMemo(() => {
    ...
    return movies
      .filter(m => m.ottDate) // only movies with OTT info
      ...
      .sort((a, b) => {
        ...
        return new Date(b.ottDate!).getTime() - new Date(a.ottDate!).getTime()
      })
    ...
    <section>
      ...
      <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
        <div>
          ...
          <p className="text-muted text-xs mt-0.5">
            Latest Tamil &amp; dubbed releases on Netflix, Prime, Hotstar &amp; more
          </p>
        </div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-2.5 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-red-500/40 focus:ring-1 focus:ring-red-500/20 transition-all"
      ...
      <div className="flex gap-2 flex-wrap mb-3">
        {PLATFORMS.map(p => {
          ...
          return (
            ...
            >
              {c && <span className="mr-1">{c.short}</span>}{p}
            </button>
          )
        })}
      </div>
      ...
      {results.length === 0 ? (
        <div className="text-center py-12 text-slate-600">
          ...
          <p className="text-sm">No movies found. Try a different search or filter.</p>
        </div>
      ) : (
              ...
              <Link key={movie.id} href={`/movies/${movie.slug}`} className="group block">
                <div
                  ...
                  style={{
                    ...
                    boxShadow: isSoon ? '0 0 16px rgba(245,158,11,0.07)' : undefined,
                  }}
                >
                  {/* Gradient top strip from movie palette */}
                  ...
                  <div className="p-3.5 flex flex-col gap-2 flex-1">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      {movie.badge && (
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-gold-500/12 border border-gold-500/20 text-gold-400 font-bold whitespace-nowrap flex-shrink-0">
                          {movie.badge}
                        </span>
                      )}
                    </div>

                    {/* Director · Year · Language */}
                    <p className="text-slate-500 text-[10px]">
                      {movie.director} · {movie.year}
                      {movie.originalLanguage && (
                        <span className="text-cyan-600/80"> · {movie.originalLanguage}</span>
                      )}
                    </p>
                    ...
                    {/* OTT row — spacer pushes to bottom */}
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: `0 0 20px ${cfg ? cfg.bg : 'rgba(245,158,11,0.4)'}`,
                      }}
                    >
      ...
      {results.length > 8 && (
        <div className="text-center mt-4">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-semibold text-slate-400 border border-white/8 hover:border-white/15 hover:text-white transition-all"
          >
            {showAll
